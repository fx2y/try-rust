fn main() {
    ...
    let config_and_const_values = {
        ...
        // 4. Load an immutable config structure from the string.
        config_text.parse::<toml::Value>().unwrap()
    };

    // 5. Show the whole config structure.
    println!("Original: {:#?}", config_and_const_values);

    // 6. Get and show one config value.
    println!("[Postgresql].Database: {}", config_and_const_values.get("postgresql").unwrap().get("database").unwrap().as_str().unwrap());
}
