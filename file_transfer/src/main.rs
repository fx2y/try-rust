...
use futures::{future::{Future, ok}, Stream};
use rand::prelude::*;
...
fn delete_file(info: web::Path<(String, )>) -> impl Responder {
    ...
    match std::fs::remove_file(&filename) {
        ...
        Err(error) => {
            println!("Failed to delete filee \"{}\": {}", filename, error);
...
fn upload_specified_file(payload: web::Payload, info: web::Path<(String, )>) -> impl Future<Item=HttpResponse, Error=Error> {
    ...
    // Get the asynchronously from the client the contents to write into the file.
    payload
        .map_err(Error::from)
    ...
    payload
        ...
        .and_then(move |contents| {
            ...
            loop {
                ...
                file = OpenOptions::new().write(true).create_new(true).open(&filename);
