...
fn main() {
    ...
    if let Value::Number(n) = &sales_and_products["sales"][1]["quantity"] {
        ...
        sales_and_products["sales"][1]["quantity"] = Value::Number(Number::from_f64(n.as_f64().unwrap() + 1.5).unwrap());
    }

    // Save the JSON structure into the other file.
    std::fs::write(output_path, serde_json::to_string_pretty(&sales_and_products).unwrap()).unwrap();
}