...
fn recreate_sqlite_db(sqlite_config: &Sqlite) -> rusqlite::Result<rusqlite::Connection> {
    ...
    conn.execute("CREATE TABLE Products (id INTEGER PRIMARY KEY, category TEXT NOT NULL, name TEXT NOT NULL UNIQUE)", params![])?;
    conn.execute("CREATE TABLE Sales (id text primary key, product_id integer not null references Products, sale_date bigint not null, quantity double precision not null, unit text not null)", params![])?;
    Ok(conn)
}

fn write_into_sqlite_db(conn: &rusqlite::Connection, sales_and_products: &SalesAndProducts) -> rusqlite::Result<()> {
...
fn recreate_postgresql_db(postgresql_config: &Postgresql) -> Result<postgres::Client, postgres::error::Error> {
    ...
    conn.execute("create table Products (id integer primary key, category text not null, name text not null unique)",
                 &[])?;
    conn.execute("create table Sales (id text primary key, product_id integer not null references Products, sale_date bigint not null, quantity double precision not null, unit text not null)",
                 &[])?;
    Ok(conn)
}

fn write_into_postgresql_db(conn: &mut postgres::Client, sales_and_products: &SalesAndProducts) -> Result<(), postgres::error::Error> {
    for product in &sales_and_products.products {
        conn.execute("insert into Products (id, category, name) values ($1, $2, $3)",
                     &[&product.id, &product.category, &product.name])?;
    }
    for sale in &sales_and_products.sales {
        conn.execute("insert into Sales (id, product_id, sale_date, quantity, unit) values ($1, $2, $3, $4, $5)",
                     &[&sale.id, &sale.product_id, &sale.date, &sale.quantity, &sale.unit])?;
    }
    Ok(())
}

fn open_redis_store(redis_config: &Redis) -> redis::RedisResult<redis::Connection> {
    Ok(redis::Client::open(format!("redis://{}/", redis_config.host).as_str())?.get_connection()?)
}
...
fn print_row_count_in_sqlite_db(conn: &rusqlite::Connection) -> rusqlite::Result<()> {
    for count in conn.prepare("SELECT COUNT(*) FROM Products")?.query_map(params![], |row| {
        let c: i64 = row.get(0)?;
        Ok(c)
    })?
    {
    ...
    for count in conn.prepare("SELECT COUNT(*) FROM Sales")?.query_map(params![], |row| {
        let c: i64 = row.get(0)?;
        Ok(c)
    })?
    {
...
fn print_row_count_in_postgresql_db(conn: &mut postgres::Client) -> Result<(), postgres::error::Error> {
...
fn main() {
    // Define the config structure by reading the TOML file specified in the command line.
    let config: Config = {
